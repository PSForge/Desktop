...
try {
  ...
  {id:'export-database-sizes',title:'Export Database Sizes Report',description:'List all mailbox databases and their sizes',category:'Database Management',parameters:[{name:'exportPath',label:'Export Path',type:'text',required:true,placeholder:'C:\\\\Reports\\\\DatabaseSizes.csv'}],scriptTemplate:p=>{const exportPath=escapePowerShellString(p.exportPath);return `Add-PSSnapin Microsoft.Exchange.Management.PowerShell.SnapIn\ntry{Write-Host "Collecting database sizes..." -ForegroundColor Cyan;$Databases=Get-MailboxDatabase -Status;$Results=$Databases|Select Name,Server,@{N='DatabaseSizeGB';E={[math]::Round($_.DatabaseSize.ToBytes()/1GB,2)}},@{N='AvailableNewMailboxSpaceGB';E={[math]::Round($_.AvailableNewMailboxSpace.ToBytes()/1GB,2)}},Mounted;$Results|Export-Csv "${exportPath}" -NoTypeInformation;Write-Host "✓ Database sizes exported: $($Results.Count) databases" -ForegroundColor Green}catch{Write-Error $_}`;}},
  {id:'configure-database-backup',title:'Configure Database Backup Settings',description:'Set circular logging and backup schedule',category:'Database Management',parameters:[{name:'databaseName',label:'Database Name',type:'text',required:true},{name:'circularLogging',label:'Enable Circular Logging',type:'checkbox',required:false,defaultValue:false}],scriptTemplate:p=>{const dbName=escapePowerShellString(p.databaseName);const circularLogging=p.circularLogging===true||p.circularLogging==='true';return `Add-PSSnapin Microsoft.Exchange.Management.PowerShell.SnapIn\ntry{Write-Host "Configuring backup settings for ${dbName}" -ForegroundColor Cyan;Set-MailboxDatabase -Identity "${dbName}" -CircularLoggingEnabled $${circularLogging};Write-Host "✓ Circular logging: ${circularLogging}" -ForegroundColor Green;Write-Host "  Run full backup to apply changes" -ForegroundColor Yellow}catch{Write-Error $_}`;}},
  {id:'test-mail-flow',title:'Test Mail Flow',description:'Send test message to verify mail flow',category:'Mail Flow',parameters:[{name:'sender',label:'Sender Address',type:'text',required:true,placeholder:'admin@contoso.com'},{name:'recipient',label:'Recipient Address',type:'text',required:true,placeholder:'test@contoso.com'}],scriptTemplate:p=>{const sender=escapePowerShellString(p.sender);const recipient=escapePowerShellString(p.recipient);return `Add-PSSnapin Microsoft.Exchange.Management.PowerShell.SnapIn\ntry{Write-Host "Testing mail flow from ${sender} to ${recipient}" -ForegroundColor Cyan;Test-Mailflow -TargetEmailAddress "${recipient}";Send-MailMessage -From "${sender}" -To "${recipient}" -Subject "Mail Flow Test - $(Get-Date)" -Body "This is a test message generated by PSForge" -SmtpServer localhost;Write-Host "✓ Test message sent successfully" -ForegroundColor Green}catch{Write-Error $_}`;}},
  {id:'export-queue-stats',title:'Export Queue Statistics',description:'Report on message queues and delivery status',category:'Mail Flow',parameters:[{name:'exportPath',label:'Export Path',type:'text',required:true,placeholder:'C:\\\\Reports\\\\QueueStats.csv'}],scriptTemplate:p=>{const exportPath=escapePowerShellString(p.exportPath);return `Add-PSSnapin Microsoft.Exchange.Management.PowerShell.SnapIn\ntry{Write-Host "Collecting queue statistics..." -ForegroundColor Cyan;$Queues=Get-Queue;$Results=$Queues|Select Identity,DeliveryType,Status,MessageCount,@{N='NextHopDomain';E={$_.NextHopDomain}},LastError;$Results|Export-Csv "${exportPath}" -NoTypeInformation;Write-Host "✓ Queue stats exported: $($Queues.Count) queues" -ForegroundColor Green;$TotalMessages=($Queues|Measure-Object -Property MessageCount -Sum).Sum;Write-Host "  Total messages in queues: $TotalMessages" -ForegroundColor Yellow}catch{Write-Error $_}`;}},
  {id:'configure-database-maintenance',title:'Configure Database Maintenance Schedule',description:'Set online maintenance schedule for database defragmentation',category:'Database Management',parameters:[{name:'databaseName',label:'Database Name',type:'text',required:true},{name:'maintenanceSchedule',label:'Maintenance Schedule',type:'select',required:true,options:[{value:'Sun.1:00 AM-Sun.5:00 AM',label:'Sunday 1-5 AM'},{value:'Sat.11:00 PM-Sun.6:00 AM',label:'Saturday 11PM-Sunday 6AM'},{value:'Daily.2:00 AM-Daily.6:00 AM',label:'Daily 2-6 AM'}],defaultValue:'Sun.1:00 AM-Sun.5:00 AM'}],scriptTemplate:p=>{const dbName=escapePowerShellString(p.databaseName);const schedule=escapePowerShellString(p.maintenanceSchedule||'Sun.1:00 AM-Sun.5:00 AM');return `Add-PSSnapin Microsoft.Exchange.Management.PowerShell.SnapIn\ntry{Write-Host "Configuring maintenance schedule for ${dbName}" -ForegroundColor Cyan;Set-MailboxDatabase -Identity "${dbName}" -MaintenanceSchedule "${schedule}";Write-Host "✓ Maintenance schedule configured" -ForegroundColor Green;Write-Host "  Schedule: ${schedule}" -ForegroundColor Yellow}catch{Write-Error $_}`;}}
];
