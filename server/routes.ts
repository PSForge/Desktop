...
Sitemap: ${baseUrl}/sitemap.xml`;
  ...
  // ========== User Stats Routes (Conversion Tracking) ==========
  
  // Get current user's stats
  app.get("/api/user-stats", requireAuth, async (req, res) => {
    try {
      const user = req.user as User;
      const stats = await storage.getOrCreateUserStats(user.id);
      return res.json(stats);
    } catch (error) {
  ...
  // Track script generation (called when script is generated)
  app.post("/api/user-stats/track-generation", requireAuth, async (req, res) => {
    try {
      const user = req.user as User;
      const { source } = req.body;
      
      // Validate source
      const validSources = ['gui', 'ai', 'wizard', 'direct'] as const;
      if (!source || !validSources.includes(source)) {
        return res.status(400).json({ error: "Invalid source. Must be one of: gui, ai, wizard, direct" });
      }
      
      const stats = await storage.incrementScriptsGenerated(user.id, source);
      return res.json(stats);
    } catch (error) {
      console.error("Track generation error:", error);
      return res.status(500).json({ error: "Failed to track script generation" });
    }
  });

  // Track script saved (called when script is saved to library)
  app.post("/api/user-stats/track-save", requireAuth, async (req, res) => {
    try {
      const user = req.user as User;
      const stats = await storage.incrementScriptsSaved(user.id);
      
      // Check for milestone achievements
      const milestoneThresholds = [5, 10, 25, 50, 100];
      const newMilestone = milestoneThresholds.find(
        threshold => stats.scriptsSaved >= threshold && !stats.milestonesAchieved.includes(threshold)
      );
      
      if (newMilestone) {
        const updatedStats = await storage.addMilestone(user.id, newMilestone);
        return res.json({ ...updatedStats, newMilestone });
      }
      
      return res.json(stats);
    } catch (error) {
      console.error("Track save error:", error);
      return res.status(500).json({ error: "Failed to track script save" });
    }
  });

  // Dismiss a nudge
  app.post("/api/user-stats/dismiss-nudge", requireAuth, async (req, res) => {
    try {
      const user = req.user as User;
      ...
      const stats = await storage.dismissNudge(user.id, nudgeId);
      return res.json(stats);
    } catch (error) {
  ...
  // Update last nudge shown
  app.post("/api/user-stats/nudge-shown", requireAuth, async (req, res) => {
    try {
      const user = req.user as User;
      const stats = await storage.updateLastNudgeShown(user.id);
      return res.json(stats);
    } catch (error) {
      console.error("Update nudge shown error:", error);
      return res.status(500).json({ error: "Failed to update nudge timestamp" });
    }
  });

  // Admin Analytics: Get milestone distribution
  app.get("/api/admin/stats/milestones", requireAdmin, async (req, res) => {
    try {
      const distribution = await storage.getMilestoneDistribution();
      return res.json(distribution);
    } catch (error) {
      console.error("Get milestone distribution error:", error);
      return res.status(500).json({ error: "Failed to retrieve milestone distribution" });
    }
  });

  // Admin Analytics: Get generation sources breakdown
  app.get("/api/admin/stats/generation-sources", requireAdmin, async (req, res) => {
    try {
      const breakdown = await storage.getGenerationSourcesBreakdown();
      return res.json(breakdown);
    } catch (error) {
      console.error("Get generation sources breakdown error:", error);
      return res.status(500).json({ error: "Failed to retrieve generation sources breakdown" });
    }
  });
