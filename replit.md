...
## Phase 4: Conversion Optimization (v4.3 - November 2025)

### User Stats Tracking
- **Database Schema**: New `userStats` table tracking per-user metrics including scriptsGenerated, scriptsSaved, generationSources breakdown (gui/ai/wizard/direct), and milestonesAchieved
- **API Endpoints**: `/api/user-stats` GET for current stats, POST endpoints for tracking saves, generations, dismissing nudges

### Time-Value Calculator Widget
- **Location**: `client/src/components/time-value-widget.tsx`
- **Features**: Shows estimated time saved (15 min/script), money saved (at configurable hourly rate), Pro comparison for upgrade motivation
- **Display**: Progress bar showing saved time vs potential with Pro tier

### Success Milestones System
- **Location**: `client/src/hooks/use-milestones.tsx`
- **Thresholds**: 5, 10, 25, 50, 100 scripts saved
- **Features**: Celebratory toast notifications at each milestone with Pro mentions, trackScriptSaved() and trackScriptGenerated() hooks for integration

### Power User Nudges
- **Location**: `client/src/components/conversion-nudges.tsx`
- **Features**: Contextual Pro tips shown after specific user behaviors (e.g., after 3+ GUI Builder uses), dismissible nudges tracked per-user, post-generation nudge showing time comparison

### Admin Conversion Analytics
- **Location**: `client/src/components/admin-conversion-analytics.tsx`
- **Features**: Milestone distribution chart showing user progress funnel, generation sources breakdown showing how users create scripts
- **Integration**: Added to admin dashboard under "Conversion Tracking" section

### Technical Notes
- Import pattern: `useAuth` from `@/lib/auth-context`
- `apiRequest` returns Response object requiring `.json()` call for parsed data
- Milestone thresholds defined in `shared/schema.ts` as `milestoneThresholds`