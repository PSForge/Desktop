...
function asRawTasks(tasks: unknown): RawTask[] {
...
const registry: TaskRegistryEntry[] = [
  ...buildEntries(asRawTasks(adTasks), "Active Directory", "active-directory"),
  ...buildEntries(asRawTasks(mecmTasks), "MECM / SCCM", "mecm"),
  ...buildEntries(asRawTasks(exchangeOnlineTasks), "Exchange Online", "exchange-online"),
  ...buildEntries(asRawTasks(exchangeServerTasks), "Exchange Server", "exchange-server"),
  ...buildEntries(asRawTasks(azureAdTasks), "Azure AD / Entra ID", "azure-ad"),
  ...buildEntries(asRawTasks(azureResourceTasks), "Azure Resources", "azure-resources"),
  ...buildEntries(asRawTasks(hyperVTasks), "Hyper-V", "hyper-v"),
  ...buildEntries(asRawTasks(intuneTasks), "Intune", "intune"),
  ...buildEntries(asRawTasks(powerPlatformTasks), "Power Platform", "power-platform"),
  ...buildEntries(asRawTasks(teamsTasks), "Microsoft Teams", "teams"),
  ...buildEntries(asRawTasks(office365Tasks), "Office 365", "office365"),
  ...buildEntries(asRawTasks(oneDriveTasks), "OneDrive", "onedrive"),
  ...buildEntries(asRawTasks(sharePointOnlineTasks), "SharePoint Online", "sharepoint-online"),
  ...buildEntries(asRawTasks(sharePointOnPremTasks), "SharePoint On-Prem", "sharepoint-onprem"),
  ...buildEntries(asRawTasks(windows365Tasks), "Windows 365", "windows365"),
  ...buildEntries(asRawTasks(windowsServerTasks), "Windows Server", "windows-server"),
  ...buildEntries(asRawTasks(eventLogTasks), "Event Log", "event-log"),
  ...buildEntries(asRawTasks(fileSystemTasks), "File System", "file-system"),
  ...buildEntries(asRawTasks(networkingTasks), "Networking", "networking"),
  ...buildEntries(asRawTasks(processManagementTasks), "Process Management", "process-management"),
  ...buildEntries(asRawTasks(registryTasks), "Registry", "registry"),
  ...buildEntries(asRawTasks(securityManagementTasks), "Security Management", "security-management"),
  ...buildEntries(asRawTasks(servicesTasks), "Services", "services"),
  ...buildEntries(asRawTasks(vmwareTasks), "VMware", "vmware"),
  ...buildEntries(asRawTasks(veeamTasks), "Veeam", "veeam"),
  ...buildEntries(asRawTasks(nutanixTasks), "Nutanix", "nutanix"),
  ...buildEntries(asRawTasks(citrixTasks), "Citrix", "citrix"),
  ...buildEntries(asRawTasks(pdqTasks), "PDQ", "pdq"),
  ...buildEntries(asRawTasks(chocolateyTasks), "Chocolatey", "chocolatey"),
  ...buildEntries(asRawTasks(servicenowTasks), "ServiceNow", "servicenow"),
  ...buildEntries(asRawTasks(connectwiseTasks), "ConnectWise", "connectwise"),
  ...buildEntries(asRawTasks(awsTasks), "AWS", "aws"),
  ...buildEntries(asRawTasks(gcpTasks), "GCP", "gcp"),
  ...buildEntries(asRawTasks(crowdstrikeTasks), "CrowdStrike", "crowdstrike"),
  ...buildEntries(asRawTasks(sophosTasks), "Sophos", "sophos"),
  ...buildEntries(asRawTasks(oktaTasks), "Okta", "okta"),
  ...buildEntries(asRawTasks(duoTasks), "Duo Security", "duo"),
  ...buildEntries(asRawTasks(fortinetTasks), "Fortinet", "fortinet"),
  ...buildEntries(asRawTasks(ciscoTasks), "Cisco", "cisco"),
  ...buildEntries(asRawTasks(netappTasks), "NetApp", "netapp"),
  ...buildEntries(asRawTasks(jamfTasks), "JAMF", "jamf"),
  ...buildEntries(asRawTasks(slackTasks), "Slack", "slack"),
  ...buildEntries(asRawTasks(zoomTasks), "Zoom", "zoom"),
  ...buildEntries(asRawTasks(githubTasks), "GitHub", "github"),
  ...buildEntries(asRawTasks(splunkTasks), "Splunk", "splunk"),
  ...buildEntries(asRawTasks(dockerTasks), "Docker", "docker"),
  ...buildEntries(asRawTasks(jiraTasks), "Jira", "jira"),
  ...buildEntries(asRawTasks(salesforceTasks), "Salesforce", "salesforce"),
  ...buildEntries(asRawTasks(sqlServerTasks), "SQL Server", "sql-server"),
];
