...
export class DatabaseStorage implements IStorage {
  ...
  async getOrCreateUserStats(userId: string): Promise<UserStats> {
    ...
    const result = await this.db.insert(userStats).values({
      ...
      scriptsGenerated: 0,
      scriptsSaved: 0,
      generationSources: { gui: 0, ai: 0, wizard: 0, direct: 0 },
      milestonesAchieved: [],
      nudgesDismissed: [],
    }).returning();
    return this.convertTimestamps(result[0]);
  ...
  async incrementScriptsGenerated(userId: string, source: keyof GenerationSources): Promise<UserStats> {
    const stats = await this.getOrCreateUserStats(userId);
    const newSources = { ...stats.generationSources };
    newSources[source] = (newSources[source] || 0) + 1;
    
    const result = await this.db.update(userStats)
      .set({
        scriptsGenerated: stats.scriptsGenerated + 1,
        generationSources: newSources,
        updatedAt: new Date(),
      })
      .where(eq(userStats.userId, userId))
      .returning();
  ...
  async incrementScriptsSaved(userId: string): Promise<UserStats> {
    const stats = await this.getOrCreateUserStats(userId);
    
    const result = await this.db.update(userStats)
      .set({
        scriptsSaved: stats.scriptsSaved + 1,
        updatedAt: new Date(),
      })
      .where(eq(userStats.userId, userId))
      .returning();
  ...
  async addMilestone(userId: string, milestone: number): Promise<UserStats> {
    const stats = await this.getOrCreateUserStats(userId);
    
    // Don't add duplicate milestones
    if (stats.milestonesAchieved.includes(milestone)) {
      return stats;
    }
    
    const newMilestones = [...stats.milestonesAchieved, milestone];
  ...
  async dismissNudge(userId: string, nudgeId: string): Promise<UserStats> {
    const stats = await this.getOrCreateUserStats(userId);
    
    // Don't add duplicate dismissals
    if (stats.nudgesDismissed.includes(nudgeId)) {
      return stats;
    }
    
    const newDismissed = [...stats.nudgesDismissed, nudgeId];
    
    const result = await this.db.update(userStats)
      .set({
        nudgesDismissed: newDismissed,
        updatedAt: new Date(),
      })
      .where(eq(userStats.userId, userId))
      .returning();
  ...
  async updateLastNudgeShown(userId: string): Promise<UserStats> {
    await this.getOrCreateUserStats(userId);
    
    const result = await this.db.update(userStats)
      .set({
        lastNudgeShown: new Date(),
        updatedAt: new Date(),
      })
      .where(eq(userStats.userId, userId))
      .returning();
  ...
  async getMilestoneDistribution(): Promise<{milestone: number; count: number; percentage: number}[]> {
    const allStats = await this.db.select({
      milestonesAchieved: userStats.milestonesAchieved,
    }).from(userStats);

    const distribution: Record<number, number> = {};
    
    // Initialize all milestone thresholds with 0
    for (const threshold of milestoneThresholds) {
      distribution[threshold] = 0;
    }
    
    // Count users who have achieved each milestone
    for (const stat of allStats) {
      const milestones = stat.milestonesAchieved || [];
      for (const milestone of milestones) {
        if (distribution[milestone] !== undefined) {
          distribution[milestone]++;
        }
      }
    }
    
    const totalUsers = allStats.length || 1; // Avoid division by zero
  ...
  async getGenerationSourcesBreakdown(): Promise<{source: string; count: number; percentage: number}[]> {
    const allStats = await this.db.select({
      generationSources: userStats.generationSources,
    }).from(userStats);
    ...
    for (const stat of allStats) {
      const sources = stat.generationSources || { gui: 0, ai: 0, wizard: 0, direct: 0 };
      totals.gui += sources.gui || 0;
      totals.ai += sources.ai || 0;
      totals.wizard += sources.wizard || 0;
      totals.direct += sources.direct || 0;
    }
    
    const totalCount = totals.gui + totals.ai + totals.wizard + totals.direct;
