...
${scriptBody}
  ...
  {
    id: 'azure-configure-site-recovery',
    title: 'Configure Azure Site Recovery',
    description: 'Set up ASR for VM disaster recovery and replication',
    category: 'Common Admin Tasks',
    isPremium: true,
    parameters: [
      {
        ...
        type: 'text',
        required: true,
        placeholder: 'RecoveryVault-Production',
        helpText: 'Name for the Recovery Services vault'
      },
      {
        name: 'resourceGroupName',
        label: 'Resource Group',
        type: 'text',
        required: true,
        placeholder: 'rg-dr-recovery',
        helpText: 'Resource group for the vault'
      },
      {
        name: 'location',
        label: 'Location',
        ...
        helpText: 'Azure region for disaster recovery'
      }
    ],
    scriptTemplate: (params) => {
      const vaultName = escapePowerShellString(params.vaultName);
      ...
      return `# Configure Azure Site Recovery
# Generated by PSForge
...
try {
    ...
    Write-Host "  1. Configure replication policy" -ForegroundColor Gray
    Write-Host "  2. Enable replication for VMs" -ForegroundColor Gray
    Write-Host "  3. Test failover to validate DR setup" -ForegroundColor Gray
    
} catch {
    Write-Error "Failed to configure Site Recovery: $_"
}`;
    }
  },

  {
    id: 'azure-manage-traffic-manager',
    title: 'Manage Azure Traffic Manager',
    description: 'Create Traffic Manager profiles and configure endpoints',
    category: 'Common Admin Tasks',
    isPremium: true,
    parameters: [
      {
        name: 'profileName',
        label: 'Traffic Manager Profile Name',
        type: 'text',
        required: true,
        placeholder: 'tm-global-app',
        helpText: 'Name for Traffic Manager profile'
      },
      {
        name: 'resourceGroupName',
        label: 'Resource Group',
    ...
    scriptTemplate: (params) => {
      ...
      return `# Manage Azure Traffic Manager
# Generated by PSForge

Connect-AzAccount

try {
    Write-Host "Creating Traffic Manager profile: ${profileName}" -ForegroundColor Cyan
    ...
    Write-Host ""
    Write-Host "Next: Add endpoints with Add-AzTrafficManagerEndpointConfig" -ForegroundColor Yellow
    
} catch {
    Write-Error "Failed to create Traffic Manager profile: $_"
}`;
    }
  },

  {
    id: 'azure-configure-expressroute',
    title: 'Configure Azure ExpressRoute',
    description: 'Set up dedicated connections to Azure',
    category: 'Common Admin Tasks',
    isPremium: true,
    parameters: [
      {
        ...
        type: 'text',
        required: true,
        placeholder: 'er-onprem-connection'
      },
      {
        name: 'resourceGroupName',
        label: 'Resource Group',
    ...
    scriptTemplate: (params) => {
      const circuitName = escapePowerShellString(params.circuitName);
      const rgName = escapePowerShellString(params.resourceGroupName);
      const location = escapePowerShellString(params.location);

      return `# Configure Azure ExpressRoute
# Generated by PSForge

Connect-AzAccount

try {
    Write-Host "Creating ExpressRoute circuit: ${circuitName}" -ForegroundColor Cyan
    ...
    Write-Host "  Service Key: $($Circuit.ServiceKey)" -ForegroundColor Yellow
    
    Write-Host ""
    Write-Host "Provide Service Key to connectivity provider to complete setup" -ForegroundColor Yellow
    
} catch {
    Write-Error "Failed to create ExpressRoute circuit: $_"
}`;
    }
  },
  ...
  {
    id: 'azure-manage-aks',
    title: 'Manage Azure Kubernetes Service (AKS)',
    description: 'Create AKS clusters and manage node pools',
    category: 'Common Admin Tasks',
    isPremium: true,
    parameters: [
      {
    ...
    scriptTemplate: (params) => {
      ...
      return `# Manage Azure Kubernetes Service (AKS)
# Generated by PSForge
...
try {
    ...
    Write-Host ""
    Write-Host "Get credentials with: az aks get-credentials -g ${rgName} -n ${clusterName}" -ForegroundColor Yellow
  ...
  {
    id: 'azure-configure-front-door',
    title: 'Configure Azure Front Door',
    description: 'Set up CDN, WAF, and load balancing',
    category: 'Common Admin Tasks',
    isPremium: true,
    parameters: [
      {
    ...
    scriptTemplate: (params) => {
      const frontDoorName = escapePowerShellString(params.frontDoorName);
      const rgName = escapePowerShellString(params.resourceGroupName);
      const backendAddress = escapePowerShellString(params.backendAddress);

      return `# Configure Azure Front Door
# Generated by PSForge

Connect-AzAccount

try {
    Write-Host "Creating Azure Front Door: ${frontDoorName}" -ForegroundColor Cyan
    
    # Create backend pool
    $BackendPool = New-AzFrontDoorBackendPoolObject \`
        -Name "backend-pool" \`
        -Backend (New-AzFrontDoorBackendObject -Address "${backendAddress}")
    ...
    Write-Error "Failed to configure Front Door: $_"
}`;
    }
  },

  {
    id: 'azure-manage-bastion',
    title: 'Manage Azure Bastion',
    description: 'Configure secure RDP/SSH access',
    category: 'Common Admin Tasks',
    isPremium: true,
    parameters: [
      {
        name: 'bastionName',
        label: 'Bastion Host Name',
        type: 'text',
        required: true,
        placeholder: 'bastion-production'
      },
      {
        name: 'resourceGroupName',
        label: 'Resource Group',
    ...
    scriptTemplate: (params) => {
      const bastionName = escapePowerShellString(params.bastionName);
      const rgName = escapePowerShellString(params.resourceGroupName);
      const vnetName = escapePowerShellString(params.vnetName);

      return `# Manage Azure Bastion
# Generated by PSForge
...
try {
    ...
    # Create or get AzureBastionSubnet
    $SubnetConfig = Get-AzVirtualNetworkSubnetConfig -Name "AzureBastionSubnet" -VirtualNetwork $VNet -ErrorAction SilentlyContinue
    
    if (-not $SubnetConfig) {
        Write-Host "Creating AzureBastionSubnet..." -ForegroundColor Yellow
        Add-AzVirtualNetworkSubnetConfig -Name "AzureBastionSubnet" -VirtualNetwork $VNet -AddressPrefix "10.0.255.0/27"
    ...
    Write-Error "Failed to create Bastion: $_"
}`;
    }
  },

  {
    id: 'azure-configure-policy',
    title: 'Configure Azure Policy',
    description: 'Create custom policies and compliance initiatives',
    category: 'Common Admin Tasks',
    isPremium: true,
    parameters: [
      {
        ...
        type: 'text',
        required: true,
        placeholder: 'require-tags-policy'
      },
      {
        name: 'policyType',
        label: 'Policy Type',
        ...
        required: true,
        options: [
          { value: 'Audit', label: 'Audit (Log violations)' },
          { value: 'Deny', label: 'Deny (Block violations)' },
          { value: 'Append', label: 'Append (Add values)' }
        ],
        defaultValue: 'Audit'
      }
    ],
    scriptTemplate: (params) => {
      ...
      return `# Configure Azure Policy
# Generated by PSForge

Connect-AzAccount

try {
    Write-Host "Creating Azure Policy: ${policyName}" -ForegroundColor Cyan
    
    # Define policy rule (example: require tags on resources)
    $PolicyRule = @"
{
    "if": {
        "allOf": [
...
"@
    ...
    Write-Host ""
    Write-Host "Assign policy with: New-AzPolicyAssignment -Name 'assignment-name' -PolicyDefinition \$PolicyDefinition" -ForegroundColor Yellow
...

  {
    ...
    description: 'Delete an Azure Resource Group and all contained resources with safety confirmations',
    category: 'Resource Management',
    isPremium: true,
    instructions: `**How This Task Works:**
This script safely deletes Azure Resource Groups and all contained resources with inventory preview and confirmation steps for governance compliance.
...
- Whether to force deletion without confirmation (checkbox)

**What the Script Does:**
- Connects to Azure account
- Retrieves Resource Group and validates it exists
- Lists all resources that will be deleted (inventory preview)
- Prompts for confirmation before deletion
- Removes Resource Group and ALL contained resources
- Reports deletion success with resource count

**Important Notes:**
- DESTRUCTIVE OPERATION - Cannot be undone
- Deletes ALL resources within the Resource Group
- Essential for cleanup of dev/test/POC environments
- Use for cost optimization and resource lifecycle management
- Always review resource inventory before confirming
- Consider exporting resource inventory first for audit trail
- Check for resource locks that may prevent deletion
- Coordinate with resource owners before production cleanup`,
    parameters: [
      {
        name: 'resourceGroupName',
        label: 'Resource Group Name',
        type: 'text',
        required: true,
        placeholder: 'rg-dev-eastus',
        helpText: 'Name of the resource group to delete'
      },
      {
        name: 'forceDelete',
        label: 'Force Delete (Skip Confirmation)',
        type: 'checkbox',
        required: false,
        defaultValue: false,
        helpText: 'Skip confirmation prompt (use with caution)'
      }
    ],
    scriptTemplate: (params) => {
      const rgName = escapePowerShellString(params.resourceGroupName);
      const forceDelete = params.forceDelete === true;

      return `# Delete Azure Resource Group
# Generated by PSForge
...
try {
    ...
    Write-Host "\\nResource Group Details:" -ForegroundColor Yellow
    Write-Host "  Name: $($ResourceGroup.ResourceGroupName)"
    Write-Host "  Location: $($ResourceGroup.Location)"
    Write-Host "  Resources to delete: $($Resources.Count)"
    
    if ($Resources.Count -gt 0) {
        Write-Host "\\nResources that will be DELETED:" -ForegroundColor Red
        $Resources | ForEach-Object {
            Write-Host "  - $($_.ResourceType): $($_.Name)" -ForegroundColor Gray
        }
    }
    
    ${forceDelete ? '' : `
    # Prompt for confirmation
    Write-Host "\\n*** WARNING: This action is IRREVERSIBLE! ***" -ForegroundColor Red
    $Confirm = Read-Host "Type 'DELETE' to confirm deletion of ${rgName}"
    ...
    Write-Host "\\nDeleting Resource Group: ${rgName}..." -ForegroundColor Cyan
  ...
  {
    id: 'azure-get-vm-performance-metrics',
    title: 'Get VM Performance Metrics',
    description: 'Retrieve CPU, memory, and disk performance metrics for Azure VMs for capacity planning',
    category: 'Monitoring & Logging',
    isPremium: true,
    instructions: `**How This Task Works:**
This script retrieves Azure VM performance metrics including CPU, memory, and disk utilization for capacity planning, right-sizing, and performance troubleshooting.

**Prerequisites:**
- Az PowerShell module
- Azure subscription with Reader role (minimum)
- Authenticated Azure session
- Azure Diagnostics enabled on target VMs (for detailed metrics)

**What You Need to Provide:**
- Resource Group name
- VM name to analyze
- Time range for metrics collection (hours)

**What the Script Does:**
- Connects to Azure account
- Retrieves VM and validates it exists
- Queries Azure Monitor for CPU percentage metrics
- Collects available memory and disk metrics
- Calculates average, minimum, and maximum values
- Displays formatted performance summary
- Provides right-sizing recommendations based on usage

**Important Notes:**
- Essential for VM right-sizing and cost optimization
- Identify underutilized VMs for downsizing
- Detect overloaded VMs needing more resources
- Metrics require Azure Diagnostics extension
- Default metrics available: CPU percentage, network in/out
- Extended metrics require diagnostics: memory, disk IOPS
- Use for capacity planning and performance baselines
- Run during peak usage periods for accurate assessment`,
    parameters: [
      {
        name: 'resourceGroupName',
        label: 'Resource Group',
      ...
      },
      {
        ...
        required: true,
        placeholder: 'vm-web01',
        helpText: 'Virtual machine name to analyze'
      },
      {
        name: 'timeRangeHours',
        ...
        defaultValue: '24',
        helpText: 'Time period for metric collection'
      }
    ],
    scriptTemplate: (params) => {
      const rgName = escapePowerShellString(params.resourceGroupName);
      ...
      return `# Get Azure VM Performance Metrics
# Generated by PSForge

Connect-AzAccount

try {
    Write-Host "Collecting performance metrics for VM: ${vmName}" -ForegroundColor Cyan
    
    # Get VM details
    $VM = Get-AzVM -ResourceGroupName "${rgName}" -Name "${vmName}" -ErrorAction Stop
    Write-Host "VM Size: $($VM.HardwareProfile.VmSize)" -ForegroundColor Yellow
    
    # Calculate time range
    $EndTime = Get-Date
    $StartTime = $EndTime.AddHours(-${timeRangeHours})
    
    Write-Host "Time Range: $($StartTime.ToString('yyyy-MM-dd HH:mm')) to $($EndTime.ToString('yyyy-MM-dd HH:mm'))" -ForegroundColor Gray
    
    # Get CPU metrics
    Write-Host "\\nFetching CPU metrics..." -ForegroundColor Cyan
    ...
    $CpuData = $CpuMetrics.Data | Where-Object { $_.Average -ne $null }
    
    if ($CpuData.Count -gt 0) {
        $CpuAvg = ($CpuData.Average | Measure-Object -Average).Average
        $CpuMax = ($CpuData.Average | Measure-Object -Maximum).Maximum
        $CpuMin = ($CpuData.Average | Measure-Object -Minimum).Minimum
        
        Write-Host "\\n=== CPU Utilization ===" -ForegroundColor Green
        Write-Host "  Average: $([math]::Round($CpuAvg, 2))%"
        Write-Host "  Maximum: $([math]::Round($CpuMax, 2))%"
        Write-Host "  Minimum: $([math]::Round($CpuMin, 2))%"
    } else {
        Write-Host "No CPU data available for the specified time range" -ForegroundColor Yellow
    }
    
    # Get Network metrics
    Write-Host "\\nFetching network metrics..." -ForegroundColor Cyan
    ...
    $NetworkOutMetrics = Get-AzMetric -ResourceId $VM.Id \`
    ...
    if ($NetworkInMetrics.Data -or $NetworkOutMetrics.Data) {
        $NetInTotal = ($NetworkInMetrics.Data.Total | Measure-Object -Sum).Sum
        $NetOutTotal = ($NetworkOutMetrics.Data.Total | Measure-Object -Sum).Sum
        
        Write-Host "\\n=== Network I/O ===" -ForegroundColor Green
        Write-Host "  Total In:  $([math]::Round($NetInTotal / 1GB, 2)) GB"
        Write-Host "  Total Out: $([math]::Round($NetOutTotal / 1GB, 2)) GB"
    }
    
    # Get Disk metrics
    Write-Host "\\nFetching disk metrics..." -ForegroundColor Cyan
    ...
    $DiskWriteMetrics = Get-AzMetric -ResourceId $VM.Id \`
    ...
    if ($DiskReadMetrics.Data -or $DiskWriteMetrics.Data) {
        $DiskReadTotal = ($DiskReadMetrics.Data.Total | Measure-Object -Sum).Sum
        $DiskWriteTotal = ($DiskWriteMetrics.Data.Total | Measure-Object -Sum).Sum
        
        Write-Host "\\n=== Disk I/O ===" -ForegroundColor Green
        Write-Host "  Total Read:  $([math]::Round($DiskReadTotal / 1GB, 2)) GB"
        Write-Host "  Total Write: $([math]::Round($DiskWriteTotal / 1GB, 2)) GB"
    }
    
    # Provide right-sizing recommendation
    Write-Host "\\n=== Right-Sizing Recommendation ===" -ForegroundColor Cyan
    if ($CpuData.Count -gt 0) {
        if ($CpuAvg -lt 10) {
            Write-Host "  Consider DOWNSIZING - CPU utilization is very low ($([math]::Round($CpuAvg, 1))%)" -ForegroundColor Yellow
        } elseif ($CpuAvg -gt 80) {
            Write-Host "  Consider UPSIZING - CPU utilization is high ($([math]::Round($CpuAvg, 1))%)" -ForegroundColor Red
        } else {
            Write-Host "  VM size appears appropriate for current workload" -ForegroundColor Green
        }
    }
